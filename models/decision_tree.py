...
from common.base_model import Classifier, BaseModel
...
class DecisionNode:
    def __init__(self, label, attribute: np.ndarray[int] = -1, children=None):
        """
...
class DecisionTree(Classifier, BaseModel):
    def __init__(self, impurity_function=ImpurityFunctions.entropy, verbose: bool = False):
        if not callable(impurity_function):
            raise InvalidArgumentException(f'split_criterion must be a callable not a {type(impurity_function)}')
    ...
    def _learn(self, features: np.array, targets: np.array, max_depth: int = None,
               attributes=None) -> DecisionNode:
        """
        train the model using features to predict targets, creating the branches will continue until a pure branch has
        been reached, no attributes are left or max_depth is reached
        :param features: features to train the model on
        ...
        if attributes is None:
            attributes = {i: np.unique(features[:, i]) for i in range(features.shape[1])}

        label, is_pure = self._get_the_most_common_target_class(targets)
        if is_pure:
            if self.verbose:
                print(f"Leaf node with label {label} was created as it has no siblings (is pure)")
            return DecisionNode(label)

        if len(attributes) == 0:
            if self.verbose:
                print(f"Leaf node with label {label} was created as attributes have been exhausted")
            return DecisionNode(label)

        if max_depth is not None and max_depth == 1:
            if self.verbose:
                print(f"Leaf node with label {label} was created as max depth has been reached")
            return DecisionNode(label)

        (index_of_maximum_impurity_reduction,
         maximum_impurity_reduction_attribute) = self._get_split_attribute(features, targets, attributes)

        values = attributes.pop(index_of_maximum_impurity_reduction)
        splits = [features[:, index_of_maximum_impurity_reduction] == value for value in values]
        branches = {}

        for value, split in zip(values, splits):
            if not np.any(split):
                if self.verbose:
                    print(f"Empty split for value {value} of attribute {maximum_impurity_reduction_attribute}")
                branches[value] = DecisionNode(label)
            else:
                if self.verbose:
                    print(f"Recursion for value {value} of attribute {maximum_impurity_reduction_attribute}")
                    if max_depth is not None:
                        branches[value] = self._learn(features[split, :], targets[split], max_depth=max_depth - 1,
                                                      attributes=attributes)
                    else:
                        branches[value] = self._learn(features[split, :], targets[split], max_depth=max_depth,
                                                      attributes=attributes)
        attributes[index_of_maximum_impurity_reduction] = values
        return DecisionNode(label, attribute=index_of_maximum_impurity_reduction, children=branches)
    ...
    def _get_the_most_common_target_class(targets):
        ...
        """
        unique_target, number_of_unique_target_occurrences = np.unique(targets, return_counts=True)
        the_most_common_target_class = unique_target[np.argmax(number_of_unique_target_occurrences)]
    ...
    def _get_split_attribute(self, features, targets, attributes):
        ...
        for attribute_index in attributes.keys():
            impurity_reduction[attribute_index] = self._get_impurity_reduction(features, attribute_index, targets)
    ...
    def _get_impurity_reduction(self, features: np.ndarray, attribute_to_reduce: np.ndarray, targets: np.ndarray):
        ...
        """
        relative_frequency = DecisionTree._get_relative_frequency(occurrences=features.shape[0],
                                                                  targets=targets)
        ...
        impurity_of_splits = self._evaluate_impurity_of_splittings(features, attribute_to_reduce, targets)
    ...
    def _evaluate_impurity_of_splittings(self, features, attribute_to_reduce, targets):
        ...
        for unique_feature in np.unique(features[:, attribute_to_reduce]):
            ...
            relative_frequency = DecisionTree._get_relative_frequency(occurrences=number_of_wanted_features,
                                                                      targets=targets,
                                                                      mask=wanted_features_mask)
            impurity = (number_of_wanted_features / features.shape[0]) * self.impurity_function(relative_frequency)
            splittings.append(impurity)
            if self.verbose:
                print(f"Impurity {impurity} calculated for the attributes {attribute_to_reduce} "
